...
void ISS::exec_step() {
	...
	switch (op) {
		...
		case Opcode::EBREAK: {
			throw ExitNotification(EXIT_SPECIAL_INSTR);
...
bool keep_csr_access(uint32_t addr, bool is_write) {
	return addr == csr::FFLAGS_ADDR || addr == csr::FRM_ADDR || addr == csr::FCSR_ADDR || addr == csr::MIP_ADDR || addr == csr::MIE_ADDR || addr == csr::MTVEC_ADDR || addr == csr::MEPC_ADDR || addr == csr::MCAUSE_ADDR || addr == csr::MHARTID_ADDR || addr == csr::MEDELEG_ADDR || addr == csr::MSCRATCH_ADDR;
}


bool ISS::is_invalid_csr_access(uint32_t csr_addr, bool is_write) {
	if (!keep_csr_access(csr_addr, is_write)) {
		throw ExitNotification(EXIT_CSR_ACCESS);
	}
...
uint32_t ISS::get_csr_value(uint32_t addr) {
	...
	switch (addr) {
		case TIME_ADDR:
		case MTIME_ADDR: {
			return total_num_instr;
		}

		case TIMEH_ADDR:
		case MTIMEH_ADDR: {
			return total_num_instr >> 32;
		}
...
void ISS::randomize_state() {
	pc = rand() & ~0b11;
	for (unsigned i=1; i<RegFile::NUM_REGS; ++i) {
		regs[i] = rand();
	}
}

void ISS::init(instr_memory_if *instr_mem, data_memory_if *data_mem, uint32_t entrypoint, uint32_t sp) {
...
void ISS::show() {
	...
	std::cout << "total-num-instr = " << std::dec << total_num_instr << std::endl;
}
